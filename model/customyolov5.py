...
class YOLOv5m(BaseModel):
    ...
    def postprocessing(self, yolo_output, conf_thres=0.5, iou_thres=0.5):
        ...
        Returns:
            clean_predictions (list): list of 3 tensors (one per head) with shape (batch_size, detected_objects, 5 + class_idx)
        """
        decoded_predictions = []
        ...
        for head in range(self.num_heads):
            ...
            # Obtain the grid of (x, y) coordinates for every anchor in the scale
            xy_grid = self.make_xy_grids(naxs, nx, ny)
            ...
            decoded_predictions.append(head_output)
        ...
        # Apply NMS
        clean_predictions = self.nms(decoded_predictions, conf_thres=conf_thres, iou_thres=iou_thres)

        return clean_predictions

    def make_xy_grids(self, naxs, nx, ny):
        """
        Creates a grid of (x, y) coordinates for every anchor in the scale.
        Parameters:
            naxs (int): number of anchors in the scale
    ...
    def nms(self, batch_predictions: torch.Tensor, conf_thres: float = 0.0, iou_thres: float = 0.0):
        """
        Recives a batch of predictions and applies non-maximum suppression to each image in the batch. But first, it
        filters out the predictions with a confidence score lower than the confidence threshold, and changes the
        coordinates from (x_center, y_center, width, height) to (x_min, y_min, x_max, y_max).

        Parameters:
            batch_predictions (torch.Tensor): tensor with shape (batch_size, N, 6)
            conf_thres (float): confidence threshold
            iou_thres (float): IoU threshold
        Returns:
            batch_selected_predictions (list): list of tensors with shape (detected_objects, x0y0x1y1 + class_idx)
        """
        ...
        for prediction in batch_predictions:
            ...
            boxes = self.coord_transf_xywh_xyxy(boxes)

            # Post confidence filtering
            conf_filtered = torch.cat((boxes, scores.unsqueeze(1), class_idx.unsqueeze(1)), 1)
    ...
    def coord_transf_xywh_xyxy(self, boxes):
        """
        Converts the coordinates from (x_center, y_center, width, height) to (x_min, y_min, x_max, y_max)
        Parameters:
            boxes (torch.Tensor): tensor with shape (N, 4)
        Returns:
            boxes (torch.Tensor): tensor with shape (N, 4)
        """
...
if __name__ == "__main__":
    ...
    start = time.time()
    post_output = model.postprocessing(out)
    end = time.time()

    print("postprocessing took {:.2f} seconds".format(end - start))
